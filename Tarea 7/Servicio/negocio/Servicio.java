...
public class Servicio
{
  ...
  public Response alta(@FormParam("usuario") Usuario usuario) throws Exception
  {
    ...
    try
    {
      ...
      //Objeto auxiliar en el manejo de RETURN_GENERATED_KEYS
      ResultSet keys=null;
      try
      {
        ...
        //Obtencion y asignacion del id generado
        keys=stmt_2.getGeneratedKeys();
        keys.next();
        int r_id=keys.getInt(1);
        usuario.id_usuario=r_id;
      }
      finally
      {
        stmt_2.close();
    ...
    //Envio de Respuesta al usuario con el id_usuario insertado
    return Response.ok().entity(j.toJson(usuario.id_usuario)).build();
  }

  //Modificacion de Metodo para realizar consulta a traves del id_usuario
  @POST
    ...
    try
    {
      ...
      try
      {
        ...
        try
        {
          ...
          return Response.status(400).entity(j.toJson(new Error("El Usuario [ ID "+id_usuario+" ] no existe"))).build();
        }
        finally
        {
          rs.close();
